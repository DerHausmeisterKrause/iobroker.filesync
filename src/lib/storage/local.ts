...
export class LocalStorageProvider implements StorageProvider {
 constructor(private readonly location:LocalLocation,private readonly randomName:()=>string=randomUUID){}
 ...
 private inside(root:string,actual:string):boolean{const relative=path.relative(root,actual);return relative===""||(relative!==".."&&!relative.startsWith(`..${path.sep}`)&&!path.isAbsolute(relative))}
 ...
 async testConnection(writeTest=false){const started=Date.now();await access(this.location.basePath);if(writeTest&&!this.location.readOnly){const p=await this.guarded(`.filesync-test-${this.randomName()}`,true);let created=false;try{await writeFile(p,"",{flag:"wx"});created=true}finally{if(created)await rm(p,{force:true})}}return {latencyMs:Date.now()-started,freeBytes:await this.getFreeSpace()}}
