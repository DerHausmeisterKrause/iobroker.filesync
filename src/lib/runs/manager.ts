...
export class RunManager {
 ...
 enqueue(jobId:string,source:RunSource,dryRun:boolean,execute:Invocation["execute"]){
  ...
  if(automatic){const pending=queue.find(item=>this.isAutomatic(item.record.source));if(pending)return{accepted:true,runId:pending.record.runId,queued:true,dryRun:pending.record.dryRun,coalesced:true}}
  else if(queue.filter(item=>!this.isAutomatic(item.record.source)).length>=10)throw new Error("Manual run queue limit reached (maximum 10 per job)");
  const record:RunRecord={runId:randomUUID(),jobId,source,dryRun,status:"queued",queuedAt:Date.now()};this.runs.set(record.runId,record);this.trim();const queued=queue.length>0||this.active.has(jobId);queue.push({record,execute});this.queues.set(jobId,queue);void this.persist();this.pump(jobId);return{accepted:true,runId:record.runId,queued,dryRun,coalesced:false}
 }
 private isAutomatic(source:RunSource){return source==="schedule"||source==="change"||source==="reconciliation"}
 ...
 private async perform(invocation:Invocation){const {record}=invocation;if(this.stopping){this.finish(record,"cancelled");return}record.status="running";record.startedAt=Date.now();await this.persist();try{const result=await invocation.execute(record.runId);record.summary=result.summary;if(result.items){this.details.set(record.runId,{runId:record.runId,items:result.items,total:result.total??result.items.length});this.trimDetails()}this.finish(record,"success")}catch(error){record.redactedError=this.redact(error);this.finish(record,"error")}await this.persist()}
