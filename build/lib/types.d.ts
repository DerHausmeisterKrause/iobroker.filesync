...
export interface LocalLocation extends LocationBase {
    type: "local";
    basePath: string;
}
export interface SmbLocation extends LocationBase {
    type: "smb";
    host: string;
    port: number;
    share: string;
...
export interface SftpLocation extends LocationBase {
    type: "sftp";
    host: string;
    port: number;
    username: string;
...
export interface PublicLocation extends Omit<LocationBase, "type"> {
...
export interface FileSyncGroup {
    id: string;
    name: string;
    description?: string;
    enabled: boolean;
...
export interface ScryptPassword {
    algorithm: "scrypt";
    salt: string;
    hash: string;
    params: {
...
export interface Job {
    ...
    targetPath: string;
    mode: "incremental" | "full" | "mirror" | "move";
    mirrorDeleteConfirmed: boolean;
    dryRun: boolean;
    recursive: boolean;
    preserveTimestamps: boolean;
    hashCheck: boolean;
    stabilitySeconds: number;
    conflict: "changed" | "always" | "never" | "error" | "version";
    trigger: {
        type: "change" | "interval" | "manual";
...
export interface AdapterConfig {
    ...
    web: WebConfig;
    webGroups: FileSyncGroup[];
    webUsers: WebUser[];
    locations: DataLocation[];
    jobs: Job[];
    groups: FileSyncGroup[];
