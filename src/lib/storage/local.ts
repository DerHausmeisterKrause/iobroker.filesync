...
export class LocalStorageProvider implements StorageProvider {
 ...
 async testConnection(writeTest=false){const started=Date.now();await this.connect();if(writeTest&&!this.location.readOnly){const p=this.resolve(`.filesync-test-${process.pid}`);await writeFile(p,"");await rm(p)}return {latencyMs:Date.now()-started,freeBytes:await this.getFreeSpace()}}
