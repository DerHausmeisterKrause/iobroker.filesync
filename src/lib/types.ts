...
export interface TransferResult { copied:number; overwritten:number; moved:number; skipped:number; failed:number; deleted:number; bytes:number; scanned:number; dryRun:boolean; totalActions:number; resultTruncated:boolean; items:Array<{path:string;action:string}>; }
