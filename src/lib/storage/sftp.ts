...
export class SftpStorageProvider implements StorageProvider {
 ...
 private inside(actual:string){if(!this.realRoot)throw new Error("SFTP root is unavailable");const relative=path.posix.relative(path.posix.resolve(this.realRoot),path.posix.resolve(actual));if(relative===".."||relative.startsWith("../")||path.posix.isAbsolute(relative))throw new Error("SFTP symlink escape denied")}
 private missing(p:string){return Object.assign(new Error(`SFTP path not found: ${p}`),{code:2})}
 private async guard(p:string,allowMissing=false){let probe=this.p(p);for(;;){try{const actual=await this.c.realPath(probe);if(actual==="")throw this.missing(probe);this.inside(actual);return}catch(error){if(!isNotFoundError(error))throw error;if(!allowMissing)throw this.missing(this.p(p));const parent=path.posix.dirname(probe);if(parent===probe)throw this.missing(this.p(p));probe=parent}}}
