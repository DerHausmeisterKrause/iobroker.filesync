...
export class SmbStorageProvider implements StorageProvider {
 private client?:SMB2;
 constructor(private readonly l:SmbLocation,private readonly secret:SecretRecord){}
 ...
 async connect(){
  ...
  this.client=new SMB2({share:`\\\\${this.l.host}\\${this.l.share}`,domain:this.l.domain??"",username:this.l.username,password:this.secret.password,port:this.l.port,autoCloseTimeout:this.l.timeoutMs});
 }
 ...
 async rename(a:string,b:string){await this.c().rename(this.p(a),this.p(b))}
 async replace(temp:string,final:string){const backup=`${final}.filesync-backup`;const existed=await this.exists(final);if(existed)await this.rename(final,backup);try{await this.rename(temp,final);if(existed)await this.remove(backup)}catch(error){if(existed&&await this.exists(backup))await this.rename(backup,final);throw error}}
 async remove(p:string){const s=await this.stat(p);if(s.type==="directory")await this.c().rmdir(this.p(p));else await this.c().unlink(this.p(p))}
}
