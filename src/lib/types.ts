...
export interface WebConfig { enabled:boolean; port:number; secure:boolean; bind:string; sessionTtlMinutes:number; certPublic?:string; certPrivate?:string; certChained?:string; }
...
export interface TransferResult { copied:number; overwritten:number; versioned:number; moved:number; skipped:number; stabilityDeferred:number; failed:number; deleted:number; bytes:number; scanned:number; dryRun:boolean; totalActions:number; resultTruncated:boolean; items:Array<{path:string;action:string;detail?:string;remainingSeconds?:number}>; }
